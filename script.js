...
function getStaticItems(w) {
  const list = STATIC_IMAGES[w] || [];
  return list.map(it => typeof it === "string" ? { src: it, caption: "" } : it);
}

const IMG_CACHE = new Map();
function warmCategory(cat, limit = 3) {
  getStaticItems(cat).slice(0, limit).forEach(({ src }) => {
...
function ensureSpecPlaceholder() {
  ...
  const container = specImg.parentElement,
    w = container?.clientWidth || specImg.clientWidth || 320,
    h = Math.round(w * 4 / 3);
...
async function loadImg(w) {
  try {
    ...
    if (localItems.length) {
      const TITLE_HINT = {
        pinterest: "pinterest", pexels: "pexels", artstation: "artstation", deviantart: "devianart",
        pixabay: "pixabay", freepik: "freepik", rawpixel: "rawpixel", unsplash: "unsplash", stocksnap: "stocksnap"
      };
      ...
      cards.forEach(card => {
        ...
        const imgEl = card.querySelector("img"),
              descEl = card.querySelector(".desc"),
              titleEl = card.querySelector(".title");
        if (imgEl) {
          if (high > 0) { imgEl.setAttribute("fetchpriority", "high"); imgEl.loading = "eager"; high--; }
          else { imgEl.setAttribute("fetchpriority", "auto"); imgEl.loading = "lazy"; }
    ...
    const pixParams = new URLSearchParams({ key: "24220239-4d410d9f3a9a7e31fe736ff62", q, lang: "pt", per_page: "9", image_type: "photo", safesearch: "true" });
    ...
    if (!results.length) {
      ...
      if (us.ok) {
        const d = await us.json(),
              uResults = Array.isArray(d.results) ? d.results : [];
    ...
    if (!results.length) {
      cards.forEach(image => {
        const imgEl = image.querySelector("img"),
              descEl = image.querySelector(".desc"),
              titleEl = image.querySelector(".title");
    ...
    cards.forEach(image => {
      const hit = results[idx % results.length],
            imgEl = image.querySelector("img"),
            descEl = image.querySelector(".desc"),
            titleEl = image.querySelector(".title");
      if (imgEl && hit?.webformatURL) imgEl.src = hit.webformatURL;
      let descText = (hit?.tags || "").toString().replace(/\s*,\s*/g, ", ").replace(/\s{2,}/g, " ");
      if (descEl) descEl.textContent = truncateText(descText, 30);
      if (titleEl) titleEl.textContent = hit?.user || "";
      idx++;
    });
...
function bindSendButton() {
  const inputEl = document.querySelector("#wordinput"),
        btnEl = document.querySelector("#wordbtn");
